...
const SettingsMenu: React.FC<SettingsMenuProps> = ({
  ...
  const startMicTest = async () => {
    ...
    try {
      ...
      levelMonitorRef.current.start(stream, setMicLevel);
      setMicTestActive(true);
      await refreshDevices();
    } catch {
      setMicTestError('Microphone blocked or unavailable.');
      setMicTestActive(false);
    }
  };
  ...
  const selectedVoice = resolveVoiceName({
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 max-h-[min(85vh,720px)] overflow-y-auto bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
          ...
          <button
            onClick={() =>
              toggleSetting(
                'language',
                settings.language === LanguagePreference.UK
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500">
                ...
                {settings.language === LanguagePreference.UK ? 'en-GB' : 'en-US'})
              </p>
            </div>
          </button>
          ...
          <button
            onClick={() =>
              toggleSetting(
                'voiceGender',
                settings.voiceGender === VoiceGender.MALE
                  ? VoiceGender.FEMALE
                  : VoiceGender.MALE
              )
            }
          ...
          <div className="px-4 py-2.5">
            <div className="flex items-center gap-3 mb-2">
              <AudioWaveform size={18} className="text-slate-400" />
              <p className="text-sm font-medium text-slate-700">AI Voice</p>
            </div>
            <select
              value={selectedVoice}
              onChange={(e) => {
                const name = e.target.value;
                const opt = GEMINI_VOICE_OPTIONS.find((v) => v.name === name);
                onUpdate({
                  ...settings,
                  voiceName: name,
                  voiceGender: opt?.gender ?? settings.voiceGender
                });
              }}
          ...
          <button
            onClick={() =>
              toggleSetting(
                'interviewMode',
                settings.interviewMode === InterviewMode.BEGINNER
                  ? InterviewMode.ADVANCED
          ...
          <div className="px-4 py-2.5">
            ...
            <select
              ...
              onChange={(e) => {
                stopMicTest();
                toggleSetting('audioInputDeviceId', e.target.value || undefined);
              }}
              className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/30"
            >
            ...
            <p className="text-[11px] text-slate-500 mt-1.5 leading-snug">
              Pick your USB mic here. Chrome recommended for live audio.
            </p>
            <div className="mt-2 flex items-center gap-2">
              <button
                type="button"
                onClick={() => void (micTestActive ? stopMicTest() : startMicTest())}
                className="text-xs font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-700"
              >
            ...
            {micTestError && (
              <p className="text-[11px] text-red-600 mt-1">{micTestError}</p>
            )}
          </div>
          ...
          <div className="px-4 py-2.5 flex items-center gap-3">
            <Mic size={18} className="text-slate-400" />
            <div className="flex-1">
              <p className="text-sm font-medium text-slate-700">Studio mic mode</p>
              <p className="text-xs text-slate-500">Disable AGC / noise suppression</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                className="sr-only peer"
                checked={Boolean(settings.studioMicMode)}
                onChange={(e) => toggleSetting('studioMicMode', e.target.checked)}
              />
          ...
          <div className="px-4 pb-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-3">
              ...
              neurophenomenological interviews — a first-person methodology for investigating
              altered states of consciousness with scientific rigour. Designed for researchers in
              consciousness studies, psychedelic science, contemplative traditions, and clinical
              psychotherapy, it provides AI-assisted transcription, structural analysis, and
              experiential mapping grounded in the frameworks of Francisco Varela, Claire
              Petitmengin, and the Husserlian phenomenological tradition.
            </p>
